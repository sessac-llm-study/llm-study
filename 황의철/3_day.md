...
$$
①\; P(E) \in \mathbb{R}, \; \mathbb{R}: \text{실수}
$$
...
$$
③\; P(\Omega) = 1, \; \Omega: \text{발생 가능한 모든 사건}
$$
...
- **공장 A, B, C**에서 제품을 생산합니다. 각 공장의 **가동률**과 **불량품 비율**은 다음과 같습니다:
...
- **임의로 선택된 제품이 불량품일 때**, 이 제품이 **공장 A에서 생산되었을 확률**은 얼마입니까?

---

## 📌 **1. 문제를 수식으로 표현하기**

- P(A) = 0.5  → 공장 A에서 생산될 확률  
- P(B) = 0.3  → 공장 B에서 생산될 확률  
- P(C) = 0.2  → 공장 C에서 생산될 확률  

- P(F|A) = 0.02$$ 0→ 공장 A에서 생산된 제품이 불량품일 확률  
- P(F|B) = 0.01 → 공장 B에서 생산된 제품이 불량품일 확률  
- P(F|C) = 0.03 → 공장 C에서 생산된 제품이 불량품일 확률  

우리는 **\( P(A|F) \)** (불량품일 때, 공장 A에서 생산되었을 확률)을 구해야 합니다.
...
여기서 \( P(F) \) (불량품일 확률)는 **전확률 정리**로 구할 수 있습니다:
...
1. 불량품이 생산될 확률 $$P(F) $$ 계산:

$$
P(F) = (0.02 \cdot 0.5) + (0.01 \cdot 0.3) + (0.03 \cdot 0.2)
$$

$$
= 0.01 + 0.003 + 0.006 = 0.019
$$

2. 베이즈 정리 적용:

$$
P(A|F) = \frac{0.02 \cdot 0.5}{0.019}
$$

$$
= \frac{0.01}{0.019} \approx 0.526
$$
...
임의로 선택된 제품이 **불량품일 때**, 이 제품이 **공장 A에서 생산되었을 확률**은 약 **52.6%**입니다.
...
1. 각 공장의 확률과 불량률을 곱하여 총 불량 확률을 구합니다.  
2. 베이즈 정리를 통해 원하는 조건부 확률을 계산합니다.  
